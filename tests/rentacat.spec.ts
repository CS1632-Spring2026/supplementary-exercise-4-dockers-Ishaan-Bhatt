...
test.beforeEach(async ({ page, context }) => {
  await context.addCookies([
    { name: '1', value: 'false', url: BASE_URL },
    { name: '2', value: 'false', url: BASE_URL },
    { name: '3', value: 'false', url: BASE_URL },
  ]);
...
test('TEST-1-RESET', async ({ page }) => {
  ...
  await expect(page.locator('#listing li').nth(0)).toHaveText('ID 1. Jennyanydots');
...
test('TEST-2-CATALOG', async ({ page }) => {
  ...
  await expect(page.locator('ol li').nth(1).locator('img')).toHaveAttribute('src', '/images/cat2.jpg');
});
...
test('TEST-6-RETURN', async ({ page }) => {
  ...
  await expect(page.locator('#listing li').nth(0)).toHaveText('ID 1. Jennyanydots');
  await expect(page.locator('#listing li').nth(1)).toHaveText('ID 2. Old Deuteronomy');
  await expect(page.locator('#listing li').nth(2)).toHaveText('Rented out');
